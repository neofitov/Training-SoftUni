...
def create_workflow(settings: AppSettings) -> CompiledStateGraph[WorkflowState, WorkflowContext]:
    #print model, key, and base url for debugging purposes
    print(f"Using model: {settings.openai_model}")
    print(f"Using OpenAI API key: {'set' if settings.openai_api_key else 'not set'}")
    print(f"Using OpenAI base URL: {settings.openai_base_url if settings.openai_base_url else 'default'}")    
          
    split_model = ChatOpenAI(model=settings.openai_model, api_key=settings.openai_api_key, base_url=settings.openai_base_url).with_structured_output(DelegationMatrix) # pyright: ignore[reportArgumentType]
    analyze_model = ChatOpenAI(model=settings.openai_model, api_key=settings.openai_api_key, base_url=settings.openai_base_url) # pyright: ignore[reportArgumentType]
    aggregate_model = ChatOpenAI(model=settings.openai_model, api_key=settings.openai_api_key, base_url=settings.openai_base_url) # pyright: ignore[reportArgumentType]
    ...
    graph_builder.add_node("split_work", _create_split_work_node(split_model)) # pyright: ignore[reportArgumentType]
    graph_builder.add_node("analyze", _create_analyze_node(analyze_model)) # pyright: ignore[reportArgumentType]
...
def _clone_repository(state: WorkflowState, runtime: Runtime[WorkflowContext]):
    ...
    if destination.exists():
        ...
        try:
            ...
            # Fallback for Windows WinError 32 (file locked by another process)
            if sys.platform == "win32":
                subprocess.run(
                    ["cmd", "/c", "rd", "/s", "/q", str(destination.resolve())],
                    check=True,
                )
            else:
                raise
...
def _create_split_work_node(model: ChatOpenAI):
    def _split_work(state: WorkflowState):
        ...
        return { "delegation_matrix": response.groups } # pyright: ignore[reportAttributeAccessIssue]
...
"""
        ...
        @tool
        def post_pr_comment(content: str) -> str: # pyright: ignore[reportReturnType]
