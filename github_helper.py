...
def _github_api_request(
    ...
    try:
        ...
        try:
            ...
            message = payload.get("message", error_body) # pyright: ignore[reportOptionalMemberAccess]
        except json.JSONDecodeError:
            pass

        if exc.code == 403 and message == "Resource not accessible by personal access token":
            raise RuntimeError(_build_github_permission_error(method, path)) from exc
...
def _build_github_permission_error(method: str, path: str) -> str:
    message = [
        f"GitHub API request [{method} {path}] failed with status 403: the configured GITHUB_ACCESS_TOKEN does not have enough permissions.",
    ]

    if method == "POST" and "/issues/" in path and path.endswith("/comments"):
        message.append(
            "Posting pull request comments requires a token with write access to issues or pull requests for the target repository."
        )
    else:
        message.append("The token can authenticate, but GitHub denied this operation.")

    message.append(
        "If you are using a fine-grained personal access token, grant Pull requests: Read and write or Issues: Read and write for this repository."
    )
